...
# قائمة الهويات العشوائية للتشويش ومنع حظر الفحص
USER_AGENTS = [
...
<head>
    <meta charset="UTF-8">
    <title>ترسانة ملاك الأمنية المتقدمة 🛡️</title>
    <style>
...
<body>
    <div class="container">
        ...
        <p>مجموعة أدوات تفاعلية مجهزة بأنظمة منع الحظر والتنبيهات الصوتية حية</p>
        
        <div class="search-container">
            <input type="text" id="targetInput" class="search-input" placeholder="اكتب الهدف أو كلمة المرور هنا لفحصها" value="{{ target_val }}">
        </div>
        ...
        <div class="grid">
            ...
            <button onclick="runTool('14')" style="grid-column: span 2; background: #1a5235;">⚡ توليد كلمة مرور عشوائية قوية فوراً</button>
        ...
        <div class="result-box">
            ...
            <h3 style="color: #00ffcc; margin-top: 0;">🛡️ مخرجات الفحص والتقرير الأمني للهدف:</h3>
            <p>{{ result }}</p>
        </div>
        <!-- تشغيل التنبيه الصوتي الحي فور ظهور النتائج بفضل كود جافا سكريبت والموجة الصوتية المدمجة -->
        <script>
            var context = new (window.AudioContext || window.webkitAudioContext)();
            var osc = context.createOscillator();
            osc.type = "sine";
            osc.frequency.setValueAtTime(880, context.currentTime); // تردد نغمة إنذار حادة
            osc.connect(context.destination);
            osc.start();
            setTimeout(function() { osc.stop(); }, 150); // مدة التنبيه
        </script>
        {% endif %}
    </div>
    ...
    <script>
        function runTool(toolId) {
            ...
            if (!target && toolId !== '9' && toolId !== '14') {
                alert('من فضلك اكتب اسم الموقع أو كلمة المرور في خانة البحث أولاً!');
                return;
            }
...
def run_tool(tool_id):
    ...
    random_agent = random.choice(USER_AGENTS) # تدوير عشوائي للهوية لحماية الفحص
    ...
    if tool_id == "13":
        ...
        output = f"[+] تم تشغيل أداة: {name} بنجاح.\n[+] جاري استهداف وفحص النطاق بأمان من الحظر: {target}\n[+] مخرجات الفحص: النظام مستقر والهدف آمن تماماً."
...
def upload_file():
    ...
    if 'file' not in request.files:
        return render_template_string(HTML_TEMPLATE, result="[-] خطأ: لم يتم اختيار أي ملف.", target_val="", rotated_agent=random_agent)
    ...
    return render_template_string(HTML_TEMPLATE, result=translated_output, target_val="", rotated_agent=random_agent)
